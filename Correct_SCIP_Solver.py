...
def cst_exact():
    #Teacher:
    for t in range(nProf):
        solver.Add( sum( sum(cst[b][i][t] for i in range(nStu)) for b in range(nCouncil) ) 
                == sum(st[i][t] for i in range(nStu))
        )
    #Student:
    for i in range(nStu):
        solver.Add( sum( sum(cst[b][i][t] for t in range(nProf)) for b in range(nCouncil) ) 
...
def css_exact():
    ...
    for i in range(nStu):
        solver.Add(sum( sum(css[b][i][j] for j in range(nStu)) for b in range(nCouncil))
