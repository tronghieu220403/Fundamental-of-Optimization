...
struct info
{
    ...
    int prj[1001][1001], prf[1001][1001];
    int g[1001];
};
...
int solve(int e, int f)
{
    ...
    for(int i=1;i<=g.nStu;i++)
    {
        ...
        s[i].s.clear();
        s[i].s.resize(1005);
        memset(&s[i].s[0],0,1005*sizeof(dou_ll));
        memset(&s[i].t[0],0,1005*sizeof(dou_ll));
    }
    for(int _t=1;_t<=g.nProf;_t++)
    {
        t[_t].s.clear();
        t[_t].s.resize(1005);
        memset(&t[_t].s[0],0,1005*sizeof(dou_ll));
    }
    ...
    */
    ...
    int lit[1000],lis[1000];
    bool sels[1000],selt[1000];
    fill(sels,sels+1000,0);
    fill(selt,selt+1000,0);
    int ps_sz, pt_sz;
    ...
    for(int _c=1;_c<=g.nC;_c++)
    {
        ...
        */
        ...
        {
            trash.push_back(t[mt].s[now].next);
            now = t[mt].s[now].next;
        }
        ps.resize(1005);
        memset(&ps[0],0,1005*sizeof(dou_ll));
        ps_sz = ins(ps,trash);
        trash.clear();
        int ms = -1;
        now = 0;
        ...
        */
        ...
        pt.resize(1005);
        memset(&pt[0],0,1005*sizeof(dou_ll));
        pt_sz = ins(pt,trash);
        trash.clear();
        //pt.erase(mt);
        //ps.erase(ms);
...
void input()
{
    ...
    //cout<<g.prj[462][234]<<el;
    //cout<<g.prj[462-1][234-1]<<el;
    //cout<<g.prj[462+1][234+1]<<el;
    
    int left = 1; int right = 150;
    int mid = 0;
    int _e = 1;
    int _f = 1;
    //solve(13,80)<<el;
